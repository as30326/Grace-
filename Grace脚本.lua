...
sound:Play()
    ...
    about:Toggle("白眼睛透视","Valkiry",false,function(state)
    EAT = state
        if EAT then
            _G.Tree2ESPInstances = {}
            local esptable = {doors = {}}
            ...
            local function monitorTree2()
                for _, instance in pairs(workspace:GetDescendants()) do
                    if instance:IsA("Model") and instance.Name == "Worm" then
                        createBillboard(instance, "眼睛", Color3.new(0, 255, 0)) -- Change color as needed
                    end
                end

                workspace.DescendantAdded:Connect(function(instance)
                    if instance:IsA("Model") and instance.Name == "Worm" then
                        createBillboard(instance, "眼睛", Color3.new(， , 255, 0)) -- Change color as needed
                    end
                end)
            end
